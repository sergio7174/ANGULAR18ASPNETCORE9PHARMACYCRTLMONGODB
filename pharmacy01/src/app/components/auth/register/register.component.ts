  ...
  this.authService.getAllAdmin().pipe(
    takeUntilDestroyed(this.destroyRef)).subscribe({
      next: ( response:any ) => {
        ...
        const HaveAdmin = haveData.admin;

        alert("Estoy en login.component - line 81 - HaveAdmin:  " + HaveAdmin);

        if (HaveAdmin == null ) {
          this.haveAdmin = 'true';
          alert("Estoy en login.component - line 85 - haveAdmin:  " + this.haveAdmin);
        } else {
          this.haveAdmin = 'false';
          alert("Estoy en login.component - line 87 - haveAdmin:  " + this.haveAdmin);
...
/**** my postdata new block *******************/
    ...
    //postData.append('isAdmin', this.registerForm.get('isAdmin')?.value);
    postData.append('isAdmin', this.haveAdmin);
    ...
    next: (response) => {
        ...
        //alert("Estoy en register.component - line 154 - this.mensajeBackend:  "+this.mensajeBackend);
        /*alert("Estoy en register.component - line 155 - this.newUser:  "
           +this.dataUser);*/

      if (!this.newUser) {     
        if (this.mensajeBackend){
          this.toast.error(this.mensajeBackend);
          //alert("Estoy en register.component - line 161 - this.mensajeBackend:  "+this.mensajeBackend);
        }}
